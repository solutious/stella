  ...
  module Adapter
    ...
    class PcapWatcher
      ...
      def monitor_dns
        ...
        httpdump = Pcaplet.new(:device => @device)
        
        req_filter  = Pcap::Filter.new('udp port 53', httpdump.capture)
        httpdump.add_filter(req_filter)
          
        lookup = {}
        httpdump.capture.loop(@maxpacks) do |packet|
          ...
          if dns_header.query? then
            ...
            lookup[domain_name][:request_at] = packet.time.to_f
      ...
      def monitor_http
        ...
        trap("INT") { httpdump.capture.close; exit 0; }
        
        req_filter  = Pcap::Filter.new('tcp and dst port 80', httpdump.capture)
        resp_filter = Pcap::Filter.new('tcp and src port 80', httpdump.capture)
        httpdump.add_filter(req_filter | resp_filter)
        httpdump.capture.loop(@maxpacks) do |packet|
