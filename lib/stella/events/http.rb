...
module Stella
  module Events
    ...
    module HTTP
      ...
      module DSL
        def add_http_event(meth, *args, &blk)
          if args[0].nil?
        ...
        def get(*args, &blk);    add_http_event :get,    *args, &blk; end
        def put(*args, &blk);    add_http_event :put,    *args, &blk; end
        def post(*args, &blk);   add_http_event :post,   *args, &blk; end
        def head(*args, &blk);   add_http_event :head,   *args, &blk; end
        def delete(*args, &blk); add_http_event :delete, *args, &blk; end

        def xget(*args, &blk);    skip :get, args.first     end
        def xput(*args, &blk);    skip :put, args.first     end
        def xpost(*args, &blk);   skip :post, args.first    end
        def xhead(*args, &blk);   skip :head, args.first    end
        def xdelete(*args, &blk); skip :delete, args.first  end
      end
      
      class Auth < Struct.new(:user, :pass, :domain)
        include EventAuth
      ...
      class RequestTemplate < Storable
        ...
        def auth(user=nil, pass=nil, domain=nil)
          @http_auth ||= Stella::Events::HTTP::Auth.new
